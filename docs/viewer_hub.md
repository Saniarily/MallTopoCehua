...
- ✅ 单案例：抽象拓扑图（骨架高亮）、**真实轮廓 + 真实位置关键点网络**（图 CSV 可达时）、条件字段、由图重算的拓扑指标与案例表 g_* 指标
...
- ✅ 实验结果：按族（阶段一训练 / 消融 / 阶段二评估 / 阶段二训练 / 阶段三评估 / 改造）列表，多实验对比表 + 柱状图（含 std 误差线），逐查询 / 逐样本结果，结果目录 PNG
