...
**两种工作流**：
- **改造模式（主线，`scripts/renovate_stage3.py`，F09 采用）**：当前项目多为旧商场改造而非新建。给定数据库中一座（常为低评分、网格状的）真实楼层：保留其外轮廓、阶段一原型（主走廊）与出入口；以原型为骨架、按现状规模与密度（`ConstraintSet.target_metrics.avg_degree`）用阶段二生成器重新生长次级网络（采样 `--candidates` 次，按"改造目标"择优：不新增内部断头、回路不少于现状一半、平均步行路径与整合度不劣于现状、单连通、阶段一预测评分更高）；在**同一轮廓**内嵌入：**只锚定既有出入口**（立面附近的断头关键点，真实坐标；`CorridorFitter.fit(anchors=..., entrance_targets=...)`），其余节点（含原型节点）重新布局以取得更优的拓扑/几何——v1 锚定全部骨架节点时拓扑几乎不变；候选择优目标加入**阶段一预测评分**（`make_score_fn`：用拟合好的 ranker 给「该商场条件 + 候选拓扑重算的图指标」打分）；楼层筛选 `select_floors`（≥12 节点、≥6000 m²、首层或二层、有出入口、可选评分阈值）；一键成廊；输出"现状 / 更新 / 走廊方案"三联图 + 现状 vs 更新的拓扑指标表（回路数、平均步行路径、拓扑直径、平均接近中心性、最大介数、连接度熵、断头节点、锐角比例，▲ 标改善方向；**回路数与连接度熵只报告不评判**）。阶段三拟合器对非凸轮廓（L / U）同时检查走廊**边**是否在建筑轮廓内（`_EdgeInsideChecker` / `repair_outside`），避免走廊跨越凹角越界。
- **新建模式（`evaluate_stage3.py` self / transfer）**：把网络嵌入自身或另一相近面积的轮廓，用于全库几何合规评估与校准。
...
→ render_corridors: **骨架–骨架边 = 主走廊，其余 = 次走廊**（无骨架时退化为外环/边介数）；宽度由 corridor_ratio(≈0.18) 预算反推并裁到 [3,8] m；
   ...
   **出入口**：度 1 节点（原有断头 = 主要出入口）在 entrance_reach·w(=4w) 内可达立面 → 出入口，否则 → 竖向交通核(vertical_core)；
   再从外环节点按"立面最远点"贪心补足到目标数 ≈ 立面周长 / 100 m（裁到 [2, 6]，间距 ≥ 30 m，穿越店铺带的连接段 ≤ 40 m）——出入口沿建筑四周分布而非集中；
   **中庭**：全网络中心线的面（不只主走廊）→ 收缩、去走廊、紧凑性过滤（最短边 ≥ 1.5w，长宽比 ≤ 4）→ 大块取紧凑中心（细长条取中点正方形）→
   按"紧凑度 × 面积 × 是否临主走廊"打分，目标数 ≈ 楼层面积 / 8000 m²（裁到 [1, 4]），中心间距 ≥ 0.22·√面积 —— 中庭沿商场分布而非聚在一处；
   拟合中度 1 节点受"朝内缩区边界"的力（w_leaf_facade），使原有断头落在靠近立面处成为出入口
