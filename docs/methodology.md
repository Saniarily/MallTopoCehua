...
**两种工作流**：
- **改造模式（主线，`scripts/renovate_stage3.py`，F09 采用）**：当前项目多为旧商场改造而非新建。给定数据库中一座（常为低评分、网格状的）真实楼层：保留其外轮廓、阶段一原型（主走廊）与出入口；以原型为骨架、按现状规模与密度（`ConstraintSet.target_metrics.avg_degree`）用阶段二生成器重新生长次级网络（采样 `--candidates` 次，按"改造目标"择优：不新增内部断头、回路数与平均步行路径不劣于现状、单连通）；在**同一轮廓**内嵌入：**只锚定既有出入口**（立面附近的断头关键点，真实坐标；`CorridorFitter.fit(anchors=..., entrance_targets=...)`），其余节点（含原型节点）重新布局以取得更优的拓扑/几何——v1 锚定全部骨架节点时拓扑几乎不变；候选择优目标加入**阶段一预测评分**（`make_score_fn`：用拟合好的 ranker 给「该商场条件 + 候选拓扑重算的图指标」打分）；楼层筛选 `select_floors`（≥12 节点、≥6000 m²、首层或二层、有出入口、可选评分阈值）；一键成廊；输出"现状 / 更新 / 走廊方案"三联图 + 现状 vs 更新的拓扑指标表（回路数、平均步行路径、拓扑直径、平均接近中心性、最大介数、连接度熵、断头节点、锐角比例，▲ 标改善方向）。
