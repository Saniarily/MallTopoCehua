...
## 5. 阶段二 · 真实 ShareGPT 语料（**最后 600 条留出**，seed 0）— 真实数据
...
| search_expander(16) | **98.5%** | 0.34% | 9.1% | **2.2%** | 1.04 | 54.9 / 59.9 | ~20 ms |
| ar_gnn v1（BFS 顺序、单标签 CE；第 3 轮第一次运行）| 94.3% | 0.34% | 4.9% | 12.8% | 1.02 | 55.3 / 64.4 | 0.12 s |
| ar_gnn v1 best-of-16 | 97.2% | 0.34% | 4.6% | 3.3% | – | 55.9 / 63.6 | 1.95 s |
| **ar_gnn v2**（标签顺序、集合似然、结构特征；沙箱 8 epoch）| 94.8% | 0.34% | 5.6% | 13.3% | 1.01 | **58.3 / 66.4** | 0.32 s |
| ar_gnn v2 T=0.3 | 77.7% | 0.34% | 4.1% | 26.8% | 1.01 | 59.5 / 68.7 | 0.32 s |
| ar_gnn v2 best-of-16 | 待运行（Mac）| | | | | | |

骨架边占目标边比例 = **52.8%**（任何保持骨架的生成器的召回下界）；新增边的召回：rule ≈ 3%，ar_gnn v2 ≈ 12%。

**判读**
1. **Ground truth 在大纲 5 指标下只有 94.7% 通过率、1.67 个连通分量**——低于规则生成器（98%）。大纲指标衡量的是"统计上像骨架"（密度/ASPL 保持），并不衡量"像真实商场"；因此**目标边召回/精度是区分生成方法的关键指标**，大纲指标只是合格线。论文里应把 ground truth 这一行放进表中说明这一点。
2. **v1 失败原因已定位**（训练曲线：loss 4.94→4.78，val anchor acc 0.16→0.17，12 epoch 无变化）：(a) teacher forcing 用 BFS 顺序，而语料的字母标签就是生成顺序（O→P→Q 形成挂在 C-B-A 上的走廊），BFS 把走廊拆散；(b) 31% 的步骤有 ≥2 个合法锚点，单标签 CE 给出自相矛盾的监督；(c) 节点特征无法区分"哪个骨架节点该长出下一条走廊"。
3. **v2 修复后**：val anchor acc 0.17 → **0.41**（top-3 0.68），训练速度 200 s → 72 s/epoch；目标边召回 55.3 → **58.3%**，精度 64.4 → **66.4%**，均高于 rule/search（54.9 / 59.9）——**这是第一个在结构指标上超过规则基线的生成器**。但 ASPL 偏差 13% 高于规则法（best-of-16 重排预计可压到 3% 左右，v1 经验），低温采样（T=0.3）精度更高但 ASPL 偏差恶化到 27%（走廊过长）。
4. 剩余差距：新增边召回 12% vs 上界 100%——语料中 35% 的新节点标签顺序与"骨架节点数 + k"不对齐（`letter_label` 对齐率 65.4%），即使连接对了，标签不对也不计入召回；下一步可用**结构匹配**（忽略标签的图编辑距离 / 最大公共子图）替代按标签的边匹配作为补充指标。
...
第 3 轮状态：S1-A~D 已运行（§1/§1b/§3/§4）；S2 第一次运行得到 v1 结果（§5），v2 需在 Mac 上重训并评估：`SKIP_S1=1 bash scripts/run_real_data_phase4.sh`（含 ground truth 参照、rule、search、ar_gnn、ar_gnn_greedy、ar_gnn_bestof16）。
