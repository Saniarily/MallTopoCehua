...
第 3 轮状态：S1-A~D 已运行（结果已并入 §1/§1b/§3/§4）；**S2-A/S2-B 未运行**（第一次运行时 `CORPUS` 未设置，脚本在 S2 前退出；现已改为自动从 `configs/data/legacy(.local).yaml` 的 `sharegpt_json` 读取，`SKIP_S1=1 bash scripts/run_real_data_phase4.sh` 只跑阶段二）。
